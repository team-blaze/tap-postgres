...
# double #quote, write two double quotes.)
...
def selected_value_to_singer_value_impl(elem, sql_datatype):
    ...
    if elem is None:
        ...
        # NB> We cast NaN's to NULL as wal2json does not support them and now we are at least
        # consistent(ly wrong)
        if elem.is_nan():
            cleaned_elem = None
        else:
        ...
        # NB> We cast NaN's, +Inf, -Inf to NULL as wal2json does not support them and now we are at
        # least consistent(ly wrong)
        if math.isnan(elem):
            cleaned_elem = None
